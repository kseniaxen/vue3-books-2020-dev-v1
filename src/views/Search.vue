...
div
  ...
  el-row#search-row(type="flex", justify="center", align="middle")
    ...
    el-col(:lg="2", :sm="2", :xs="24")
      el-button(
        icon="filter_list",
        @click="state.filterBarActive = !state.filterBarActive"
      )
        span Фильтр
...
import {
  computed,
  reactive,
  /* onBeforeUnmount, */ /* watch, */ onMounted,
  onUnmounted,
} from "vue";
...
export default {
  ...
  setup() {
    ...
    const suggestedTypes = computed(() =>
      store.getters.searchTypes.map((item, index, type) => {
        return {
          text: item.name,
          value: item.id,
        };
    ...
    function loadMoreBooks() {
      ...
      store
        .dispatch("loadBooks", state.filter)
        .then(() => {
    ...
    return {
      ...
      suggestedCountries,
      suggestedCities,
      suggestedTypes, // computed
      /* booksInfiniteHandler, */ onSearchInputChange,
      countryItemSelected,
      countryInputChange,
      cityItemSelected,
      cityInputChange,
